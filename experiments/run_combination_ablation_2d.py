"""Progressive layer-stack ablation (L1 → L1+L2 → … → full)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run progressive layer-stack ablation")
    parser.add_argument("--trials", type=int, default=None)
    parser.add_argument("--scenarios", nargs="+", default=None)
    parser.add_argument(
        "--config",
        type=str,
        default=str(CONFIG_DIR / "experiments" / "ablation_combination.yaml"),
    )
    ...
    for stack in exp_cfg["stacks"]:
        name = stack["name"]
        enabled = stack["enabled"]
        flags = resolve_variant(enabled_layers=enabled)
        method = f"fcem_{name}"
        ...
        for scenario in scenarios:
            ...
            for trial in range(n_trials):
                summary = run_fcem_trial(
                    ...
                    output_parts=("ablation", "combination", name),
                    extra_summary={"stack": name, "enabled_layers": ",".join(enabled)},
